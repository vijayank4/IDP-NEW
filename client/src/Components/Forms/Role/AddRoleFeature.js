...
const AddRoleFeature = (props) => {

    const [childRoleData, setChildRoleData] = useState([]);
    ...
    const handleClickChildCheckbox = (event) =>{
        ...
        if(event.target.children[0].checked)
        {
            ...
            const updatedItems = childRoleData.filter(item => item !== event.target.children[0].getAttribute('feature-id'));
            setChildRoleData(updatedItems);
            //console.log(updatedItems)
            setParentRoleData({ ...parentRoleData, [parentFeatureId]: updatedItems})
        }
        else
        {
            ...
            if(parentElement !== null)
            {
                if(parentElement.checked)
                {
                    ...
                    setChildRoleData([ ...childRoleData, event.target.children[0].getAttribute('feature-id')]);
                    setParentRoleData({ ...parentRoleData, [parentFeatureId]: [ ...childRoleData, event.target.children[0].getAttribute('feature-id')]})
                }
                else
                {
                    setShowToast(true);
    ...
    const handleClickParentCheckbox = (event) =>{
        ...
        {
            setParentRoleData({ ...parentRoleData, [feature_id]: []});
            setChildRoleData([])
        }
        else
        {
            ...
            delete parentRoleData[event.target.getAttribute('feature-id')];
            setParentRoleData(parentRoleData);
        }
    }
    ...
    const onSubmitForm = async (event) => {
        event.preventDefault();
        //console.log(props.formRoleData)
        //console.log(parentRoleData)
        const errorArr = [];
        for (const key in props.formRoleData) {
            if(props.formRoleData[key] === '' || (props.formRoleData[key] === false && key !== 'free_role'))
            {
        ...
        if(document.getElementsByClassName('is-invalid').length === 0 && errorArr.length === 0)
        {
            ...
            setDataLoading(true);
            const postData = JSON.stringify({ roleData:props.formRoleData, featureData:parentRoleData });
            //console.log(postData)
            const encryptedData = encryption(postData);
            const postUrl = global.config.PUBLIC_URL+'/api/create_role';
            try {
                            ...
                            <div className="checkbox-dropdown"> 
                                <div className="dropdown new-checkdp"> 
                                    <label className="control control-checkbox chk_fnt"> 
                                        <input type="checkbox" id="parent_role" onChange={handleClickParentCheckbox} name="parent_role" feature-id={pitem.feature_id} parent-id={pitem.parent_id} feature-type={pitem.feature_type} className="parentRoleCheckbox chk_algn"/> 
                                        ...
                                        <>
                                            ...
                                            <ul className="dropdown-menu dropdown-menu-end check-drop-menu" aria-labelledby={`dropdownMenu-${pitem.feature_id}`} style={{maxHeight:400}}>
                                                {pitem.sub_feature.map((citem, cindex) => {
                                                    return (
                                                        <li key={cindex}><Link to="#" className="small pad-bt0" parent-feature-id={pitem.feature_id} onClick={handleClickChildCheckbox}><label className="control control-checkbox chk_fnt"><input type="checkbox" id="child_role" name="child_role" parent-feature-id={pitem.feature_id} feature-id={citem.feature_id} feature-type={citem.feature_type} className="childRoleCheckbox drop-align"/>&nbsp;<div className="control__indicator"></div>{citem.display_name}</label></Link></li>
                                                    )
                                                })}
                                            </ul>
                                        </>
